...
void MRVM::performRegression()
{
   ...
   if(m_inputItems.size() && m_inputItems.first()->numForecastValues())
   {
      ...
      for(int r = 0 ; r < rows ; r++)
      {
         ...
         af::array outputArray(m_maxNumRowsPerForecastValue , m_targetMatrix.dims(1));
         af::array stDev(m_maxNumRowsPerForecastValue , m_targetMatrix.dims(1));

         int divi = m_maxNumRowsPerForecastValue > 6000 ? 6000 : m_maxNumRowsPerForecastValue;
         int maxIts = std::ceil(m_maxNumRowsPerForecastValue*1.0/ divi);
         int currentStart = 0;

         for(int sr = 0 ; sr < maxIts ; sr++)
         {
            if(currentStart + divi > m_maxNumRowsPerForecastValue)
            {
               divi = m_maxNumRowsPerForecastValue - currentStart;
            }

            af::array inp = getInputMatrix(r,currentStart, divi, false);
            ...
            af::seq rseq(currentStart , currentStart + divi-1);
            outputArray(rseq, af::span) = out;
            stDev(rseq , af::span) = std;
            currentStart = currentStart + divi;
         }
...
af::array MRVM::getInputMatrix(int valueIndex, bool training)
{
   ...
   if(training)
   {
          ...
          it != m_inputItems.end(); it ++)
      {
         ...
         {
            af::array temp = item->trainingValues(valueIndex ,0 ,1);
         ...
         {
            values(af::span,colSeq) = item->trainingValues(valueIndex, 0 , m_maxNumRowsPerTrainingValue);
         }
   ...
   else
   {
          ...
          it != m_inputItems.end(); it ++)
      {
         ...
         {
            af::array temp = item->forecastValues(valueIndex , 0 , 1);
         ...
         {
            values(af::span,colSeq) = item->forecastValues(valueIndex , 0 , m_maxNumRowsPerForecastValue);
         }
...
af::array MRVM::getInputMatrix(int valueIndex, int startRow , int length, bool training)
{
   ...
   if(training)
   {
      values = af::array(length, m_numInputCols);
          ...
          it != m_inputItems.end(); it ++)
      {
         ...
         {
            values(af::span,colSeq) = item->trainingValues(valueIndex, startRow , length);
         }
   ...
   else
   {
          ...
          it != m_inputItems.end(); it ++)
      {
         ...
         {
            af::array t =  item->forecastValues(valueIndex , startRow , length);

            int rrr = t.dims(0);
            int fff = t.dims(1);

            values(af::span,colSeq) = t;
         }
...
af::array MRVM::getOutputMatrix(int valueIndex, int startRow, int length, bool training)
{
   ...
   if(training)
   {
      values = af::array(length, m_numOutputCols);
   ...
   else
   {
          ...
          it != m_outputItems.end(); it ++)
      {
         ...
         {
            values(af::span,colSeq) = item->forecastValues(valueIndex , startRow , length);
         }
